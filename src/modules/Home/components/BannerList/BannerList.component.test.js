...
describe('Modules/Home - BannerList', () => {
  ...
  test('check render banner list item', () => {
    const wrapper = render(componentProps);
    const instance = wrapper.instance();
    const section = componentProps.data[0];
  ...
  test('check key extractor of flat list', () => {
    ...
    const instance = wrapper.instance();
    const section = componentProps.data[0];
    const result = instance.keyExtractor(section);

    expect(result).toEqual(section.sectionId);
  });
});
